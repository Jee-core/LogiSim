...
/**
 * Circuit Inspector - Compact panel to display and edit inputs/outputs for the
 * selected gate
...
public class CircuitInspector extends JPanel {
    ...
    private void forceLEDUpdates() {
        ...
        for (GateComponent gate : currentCircuit.getGates()) {
            if (gate instanceof LED) {
                ...
                System.out.println("DEBUG: LED " + led.getName() + " state - Lit: " + led.isLit() + " Input: "
                        + led.getInputVal(0));
            }
        }
    }
        ...
        try {
            ...
            for (GateComponent gate : currentCircuit.getGates()) {
                if (gate != null) {
                    gate.computeOutput();
                    System.out.println(
                            "DEBUG: Recomputed gate: " + gate.getName() + " - Output: " + gate.getOutputVal(0));
                }
            }
    ...
    private JDialog createTruthTableDialog(List<TruthTableRow> table) {
        ...
        // REMOVED: dialog.setLocationRelativeTo(this); - This was causing bottom
        // placement
     ...
     */
    private JPanel createDialogDataRow(List<String> inputNames, List<String> outputNames, TruthTableRow row,
            int rowIndex) {
    ...
    private void updateLEDState() {
        if (isLEDSelected()) {
            ...
            if (selectedGate instanceof LED) {
                ...
                led.computeOutput(); // Force LED to recompute
                System.out.println(
                        "DEBUG: LED state after computation - Lit: " + led.isLit() + " Input: " + led.getInputVal(0));
            }
