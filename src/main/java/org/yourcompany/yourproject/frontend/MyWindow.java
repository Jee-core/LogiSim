...
import org.yourcompany.yourproject.backend.businessLayer.analysis.EventHandlers;
...
public class MyWindow extends JFrame {
    ...
    public MyWindow() {
        ...
        btndel_wire = createStyledButton("🗑️Wire", BUTTON_BACKGROUND, BUTTON_FOREGROUND);
        ...
        circuitOperationsPanel.add(btndel_wire); // Empty space for alignment
        ...
        // ============ ADD THIS CONNECTION CODE ============
        // Connect GatePallete with DesignArea
        componentPalette.setCircuitPanel(circuitPanel);

        // Setup gate selection listener to notify both CircuitInspector AND GatePallete
        circuitPanel.setGateSelectionListener(new BoardHelper.GateSelectionListener() {
            @Override
            public void onGateSelected(GateComponent gate) {
                // Update CircuitInspector
                gateToolbar.setSelectedGate(gate);
                // Update GatePallete
                componentPalette.setSelectedGate(gate);
            }
        });
        ...
        btnRemComp.addActionListener(e -> {
            ...
            List<GateComponent> gates = circuitPanel.getCurrentCircuit().getGates();
            if (gates.isEmpty()) {
                JOptionPane.showMessageDialog(this,
            ...
            for (int i = 0; i < gates.size(); i++) {
                GateComponent gate = gates.get(i);
            ...
            if (selectedGateName != null) {
                ...
                if (selectedIndex != -1) {
                    GateComponent gateToRemove = gates.get(selectedIndex);
        ...
        btndel_wire.addActionListener(e -> {
            deleteSelectedWire();
        });
    ...
    private void setupPaletteListeners() {
        ...
        // LED button - ADD THIS
        componentPalette.getLedButton().addActionListener(e -> {
            addGateFromPalette("LED", 200, 150);
        });
        setupDragAndDrop(componentPalette.getLedButton(), "LED");
    }
    ...
    private void addGateFromPalette(String gateType, int x, int y) {
        ...
        GateComponent gate = createGateFromType(gateType);
        if (gate != null) {
     ...
     */
    private GateComponent createGateFromType(String gateType) {
        switch (gateType) {
            case "AND Gate":
                return new andd();
            case "OR Gate":
    ...
    private Circuit deepCopyCircuit(Circuit original) {
        ...
        // Deep copy gates
        for (GateComponent originalGate : original.getGates()) {
    ...
    private boolean getFinalCircuitOutput(Circuit circuit, Map<String, Boolean> results) {
        ...
        if (!circuit.getGates().isEmpty()) {
            GateComponent lastGate = circuit.getGates().get(circuit.getGates().size() - 1);
    ...
    private boolean addCircuitAsComponent(Circuit parentCircuit, Circuit childCircuit, String childCircuitName) {
        try {
            ...
            SubcircuitComponent circuitComponent = new SubcircuitComponent(childCircuit, childCircuitName);
            ...
            parentCircuit.addGateFunc(circuitComponent);
    ...
    private int countCircuitInputs(Circuit circuit) {
        if (circuit == null)
            return 0;
        return circuit.calculateInputCount();
    }
    ...
    private int countCircuitOutputs(Circuit circuit) {
        if (circuit == null)
            return 0;
        return circuit.calculateOutputCount();
    }
    ...
    private void loadCircuitIntoUI(Circuit circuit) {
        ...
        for (GateComponent component : circuit.getGates()) {
    ...
    private void createWireUI(Connector connector) {
        ...
        GateComponent fromGate = connector.getFromGate();
        GateComponent toGate = connector.getToGate();
        ...
        if (sourceGateUI != null && targetGateUI != null) {
            ...
            connector.setFromUI(sourceGateUI);
            connector.setToUI(targetGateUI);
        }
    }
    ...
    private void loadConnectorIntoUI(Connector connector) {
        ...
        if (sourceGateUI != null && targetGateUI != null) {
            ...
            DrawWire wire = new DrawWire(connector, sourceGateUI,
                    connector.getSourcePortIndex(), targetGateUI, connector.getDestinationPortIndex());
            add(wire);
    ...
    private DrawGates createGateUI(GateComponent component) {
    ...
    private boolean getFinalCircuitOutput(Circuit circuit) {
        ...
        if (!circuit.getGates().isEmpty()) {
            return circuit.getGates().get(0).getOutputVal(0);
        }
        ...
        for (Map.Entry<String, List<String>> projectEntry : projectCircuits.entrySet()) {
            ...
            for (String circuitName : circuits) {
                ...
                if (circuit != null) {
                    ...
                    List<GateComponent> gatesToRemove = new ArrayList<>();

                    for (GateComponent gate : circuit.getGates()) {
                        if (gate instanceof SubcircuitComponent) {
                            SubcircuitComponent circuitComp = (SubcircuitComponent) gate;
                            String referencedCircuitName = circuitComp.getCircuitName();
                            ...
                            if (circuitNameToRemove.equals(referencedCircuitName)) {
                                ...
                                System.out.println("DEBUG: Found and marking for removal: " +
                                        circuitComp.getCircuitName() + " from circuit " + circuitName);
                            }
                        }
                    }

                    // Remove the found CircuitComponent gates
                    for (GateComponent gateToRemove : gatesToRemove) {
            ...
            for (String otherCircuitName : circuits) {
                ...
                if (circuit != null) {
                    for (GateComponent gate : circuit.getGates()) {
                        if (gate instanceof SubcircuitComponent) {
                            SubcircuitComponent circuitComp = (SubcircuitComponent) gate;
                            if (circuitName.equals(circuitComp.getCircuitName())) {
                                return true;
                            }
                        }
                    }
    ...
    private void deleteSelectedWire() {
        // Find selected wire
        DrawWire selectedWire = null;
        for (Component comp : circuitPanel.getComponents()) {
            if (comp instanceof DrawWire && ((DrawWire) comp).isSelected()) {
                selectedWire = (DrawWire) comp;
                break;
            }
        }
        ...
        if (selectedWire == null) {
            JOptionPane.showMessageDialog(this,
                    "Please select a wire first by clicking on it.",
                    "No Wire Selected",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    private void removeWire(DrawWire wire) {
        if (wire == null)
            return;

        System.out.println("=== STARTING WIRE REMOVAL ===");

        Connector connector = wire.getConnector();

        // STEP 1: Remove from ALL possible business logic locations
        if (connector != null) {
            System.out.println("Removing connector: " + connector);

            // Remove from circuit
            if (currentCircuit != null) {
                // Method 1: Remove from wires list
                if (currentCircuit.getWires() != null) {
                    boolean removed = currentCircuit.getWires().remove(connector);
                    System.out.println("Removed from circuit wires: " + removed);
                }

                // Method 2: Also check if there's a getConnectors() method
                try {
                    java.util.List<Connector> connectors = currentCircuit.getConnectors();
                    if (connectors != null) {
                        boolean removed = connectors.remove(connector);
                        System.out.println("Removed from circuit connectors: " + removed);
                    }
                } catch (Exception e) {
                    System.out.println("No getConnectors method: " + e.getMessage());
                }
            }

            // Completely disconnect the connector - UPDATED METHOD NAME
            connector.disconnectWire(); // Changed from disconnect() to disconnectWire()

            // Null out all references - UPDATED METHOD NAMES
            connector.setFromGate(null); // Changed from setFromGate(null)
            connector.setToGate(null); // Changed from setToGate(null)
            connector.setFromUI(null); // Changed from setFromGateUI(null)
            connector.setToUI(null); // Changed from setToGateUI(null)
        }

        // STEP 2: Remove from ALL UI locations
        wire.setConnector(null);
        wire.setSourceGate(null);
        wire.setTargetGate(null);

        // Remove from circuitPanel
        if (wire.getParent() == circuitPanel) {
            circuitPanel.remove(wire);
            System.out.println("Removed from circuitPanel");
        }

        // Remove from wireUIComponents
        if (wireUIComponents != null) {
            wireUIComponents.remove(wire);
            System.out.println("Removed from wireUIComponents");
        }

        // STEP 3: Prevent any automatic recreation
        // Add a flag to the wire to mark it as deleted
        wire.putClientProperty("deleted", true);

        circuitPanel.revalidate();
        circuitPanel.repaint();

        System.out.println("=== WIRE REMOVAL COMPLETE ===");
    }
